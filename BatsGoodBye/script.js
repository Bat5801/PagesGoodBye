...
function updateCountdowns() {
    const farewellDate = new Date('2025-08-22T20:00:00');
...
function updateExamCountdown() {
    ...
    const examDate = new Date(`${currentYear}-06-20T09:00:00`);
    ...
    const countdownTitle = examCountdown ? examCountdown.querySelector('h3') : null;
    const countdownDisplay = examCountdown ? examCountdown.querySelector('.countdown-display') : null;

    // 分别检查每个元素是否存在
    if (!countdownTitle || !countdownDisplay) {
        console.error('无法找到中考倒计时元素');
        return;
    }

    const daysElement = document.getElementById('exam-days');
    const hoursElement = document.getElementById('exam-hours');
    const minutesElement = document.getElementById('exam-minutes');
    const secondsElement = document.getElementById('exam-seconds');

    if (timeLeft <= 0) {
        // 倒计时结束
        countdownTitle.textContent = '中考已开始';
        countdownTitle.style.fontSize = '2.5em';
        countdownTitle.style.fontWeight = 'bold';
        countdownDisplay.style.display = 'none';
    } else {
